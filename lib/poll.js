...
export default class Poll {
  ...
  help() {
    let msg = `Call ${MESSAGE_PREFIX}poll with up to 5 options in a comma delineated list.\n`;
    msg += 'Leave empty for default options\n';
    msg += `Ex) !${MESSAGE_PREFIX}poll din ding, pizza, viet \n\n`;
    msg += 'Other options:\n';
    msg += `\`${MESSAGE_PREFIX} poll help\`:  (prints this message :wave:)\n`;
    msg += `\`${MESSAGE_PREFIX} poll print\`: print out current poll (if any)\n`;
    msg += `\`${MESSAGE_PREFIX} poll close\`: close out current poll (if any)\n`;
    msg += `\`${MESSAGE_PREFIX} poll vote {num}\`: vote for an option \n`;
    msg += `\`${MESSAGE_PREFIX} poll unvote {num}\`: retract a vote for an option \n`;
    msg += `\`${MESSAGE_PREFIX} poll add {option}\`: add an option to the poll\n `;
  ...
  start(opts) {
    if (!this.state.open) {
      ...
      this.channel.send(
        `Poll already exists. Type \`${MESSAGE_PREFIX} poll print\` for options.`
      );
    }
  }
  ...
  print() {
    let msg = this.state.options.length === 0 || !this.state.open
      ...
      : `How to vote: \`${MESSAGE_PREFIX} poll vote {num}\`\n`;
